...
public partial class WallRunComponent : Node
{
	...
	public bool TryWallRun(FpsController player, float delta)
	{
		...
		// Once we're clear of every wall, forget the "blocked" wall so it can be used again.
		if (!hasWall && !_wallRunning)
		...
		if (!_wallRunning)
		{
			...
			if (hasWall && fastEnough && notRisingFast && differentWall && cooldownOk)
